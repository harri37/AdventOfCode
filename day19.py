...
patterns, desired = data.split("\n\n")

patterns = [pattern for pattern in patterns.split(", ")]
desired = [des for des in desired.split("\n")]
...
def possible(rem):
    if rem == "":
        return True
    next_patterns = [pattern for pattern in patterns if len(pattern) <= len(rem) and rem[:len(pattern)] == pattern]
    for next in next_patterns:
        if possible(rem[len(next):]):
            return True
    return False

print(sum([int(possible(des)) for des in desired]))
...
def count(rem):
    if rem == "":
        return 1
    next_patterns = [pattern for pattern in patterns if len(pattern) <= len(rem) and rem[:len(pattern)] == pattern]
    return sum([count(rem[len(next):]) for next in next_patterns])

print(sum([int(count(des)) for des in desired]))